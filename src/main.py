...
for run in range(args.runs):
    ...
    for i in range(5):
        ...
        stopper = EarlyStopping(mode='lower', filename='mainsplit-attention-comb', patience=args.patience)
        ...
        for epoch in range(args.epochs):
            ...
            aupr_batch = []
            f1_batch = []
            for d1, d2, c, y_true, fold in DataLoader(train_set, batch_size=args.batch_size, shuffle=True):
            ...
            train_loss_avg = all_loss / (len(train_set) // args.batch_size)
            p, r, acc, auc, aupr, f1 = validate_new(test_set, model)
            print(
